...
```
  프로젝트 내 유일한 포스트 팀(1명)으로
  블로그와 Post 기능을 처음부터 끝까지, 프론트엔드부터 백엔드까지 전적으로 담당하여 구현했습니다.
```  


### 블로그 기능 구현에서 중점적으로 둔 부분

```
블로그를 커뮤니티와 쇼핑몰을 연결해주는 매개체로 정의했고

블로그 기능 구현에서 가장 중요한 것은 

단순히 게시글을 작성하고 리플을 남기는 게 아니라
작성한 게시글(리뷰)와 리뷰가 게시되어 있는 어느 개인의 블로그를 통해 유저와 유저, 유저와 책이 연결되고 이어지는 것이 블로그의 핵심 역할이기 때문에 
"연결"에 중점을 두고 

전부 다 시연 할 수 없고 눈에 잘 띄지 않는 부분이라도 연결 할 수 있는 모든 부분을 촘촘하게 연결하고자 노력했습니다.


블로그를 통해(리뷰를 통해) 유저와 유저, 유저와 책이 연결되고 이어지   
...
리뷰를 통해 사용자와 사용자가 이어지고

불로그를 통해 

댓글들을 통해 사용자와 사용자가 사용자와 책이  
...
책 판매(쇼핑몰) + 유저간 책에 대해 소통하고 의견을 나누는(커뮤니티)

쇼핑몰에 커뮤니티가 더해진 홈페이지를 만들어 보기로

 블로그가 쇼핑몰과 커뮤니티를 이어주는 중간 매개체가 되서 합치는
...
+ #### Summernote 게시판 API 사용하여 에디터기능 구현 
  + ##### 게시글 사진 첨부 가능 및 다양한 글 작성 효과로 다채로운 리뷰 글 작성 가능 


+ #### 블로그 디자인 프론트앤드 작업
  + ##### 블로그 레이아웃 디자인하고 create/list/detail/update HTML 작성하여 꾸미기
 

+ #### 아이콘 추가 / blink 효과
  + ##### 새 글에는 NEW, 댓글 많은(10개 이상) 글에 HOT 아이콘 추가하여 새 글, 인기 많은 글이 눈에 잘 띄도록

+ #### 연결, 이어짐을 가장 중요하게 생각하고 작은 부분, 모두 시연할 수 없어도 구석구석 연결 가능 한 것은 모두 연갈
